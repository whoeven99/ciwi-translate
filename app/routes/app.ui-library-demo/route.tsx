...
const DEFAULT_CONFIG: DemoConfig = {
  ...
  /* eslint-disable no-restricted-syntax -- 这三个就是 --app-accent-primary* 的取值本身，配置台要把它们喂回 CSS 变量 */
  brandPrimary: "#5467ff",
...
const mediaThumbStyle: CSSProperties = {
  ...
  background: `linear-gradient(135deg, ${appAccents.primarySoft}, ${appAccents.primaryMuted}), ${appColors.surface}`,
};
...
const codeBlockStyle: CSSProperties = {
  ...
  borderRadius: appRadius.lg,
  border: `1px solid ${appColors.borderSecondary}`,
  /* eslint-disable-next-line no-restricted-syntax -- 代码块用固定深色主题，不跟随可换肤的 app token */
  background: "#0f172a",
              ...
              <Card>
                <BlockStack gap="300">
                  <InlineStack align="space-between" blockAlign="center" wrap>
                    ...
                    <InlineStack gap="200" wrap>
                      <AppPill>{config.shopName}</AppPill>
                      <AppPill>{config.targetLanguage}</AppPill>
                    </InlineStack>
                  </InlineStack>
  ...
  return (
    <div style={frameStyle}>
      <BlockStack gap="400">
        ...
        <InlineStack gap="200" wrap>
          <AppPill>Search...</AppPill>
          <AppPill>{config.targetLanguage}</AppPill>
          <AppPill>Product</AppPill>
          ...
          <Card>
            <InlineStack gap="300" wrap>
              <BlockStack gap="100">
                ...
                <AppPill>{config.sourceLanguage}</AppPill>
              </BlockStack>
              <BlockStack gap="100">
                <Text as="span" tone="subdued" variant="bodySm">
                  Target language
        ...
        <div style={sectionGrid2Style}>
          ...
          <Card>
            <BlockStack gap="300">
              ...
              <div style={sectionGrid2Style}>
                <AppMetricTile label="Translatable items" value={config.translatableItems} />
                <AppMetricTile label="Source characters" value={config.sourceCharacters} />
              </div>
            </BlockStack>
          </Card>
        </div>
    ...
    <AppMobileListCard
      ...
      rows={[
        ...
        },
        {
          ...
          value: (
            <Badge tone={autoTranslate ? "success" : "info"}>
...
export const action = async ({ request }: ActionFunctionArgs) => {
  // 配置台会写回仓库源码目录，只能在本地开发时使用；线上文件系统是临时的。
  if (isProductionNodeEnv()) {
    throw redirect("/app/translate-v4-mvp");
  }
  await authenticate.admin(request);
