...
function listDirtyFiles() {
  ...
  // 已跟踪但被 ignore 规则命中的新文件：`ls-files --others --exclude-standard` 看不到，
  // 但 `git add` 会拒。此处只返回可安全 add 的路径。
  return [...set].filter((f) => {
    const ignored = gitAllowFail(["check-ignore", "-q", f]);
    // check-ignore exit 0 = ignored；已跟踪文件通常不被 check-ignore 命中
    if (ignored.ok) {
      const tracked = gitAllowFail(["ls-files", "--error-unmatch", f]);
      return tracked.ok;
    }
    return true;
  });
...
function main() {
  ...
  if (dirty.length > 0) {
    ...
    for (const f of dirty) {
      // .cursor 等 ignore 目录下的已跟踪文件，普通 git add 仍会拒，需 -f
