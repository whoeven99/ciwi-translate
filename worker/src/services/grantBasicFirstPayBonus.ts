...
import { expireInstallTrialCreditsIfDue, getTsfDb } from "./tsfDb.js";
  ...
  await db.execute({
    sql: `UPDATE Account
          SET trialCredits = trialCredits + ?,
              trialBonusCredits = trialBonusCredits + ?,
              trialBonusExpiresAt = ?,
              trialCreditsExpiresAt = CASE
                WHEN trialInstallCredits > 0
                 AND trialInstallExpiresAt IS NOT NULL
                 AND trialInstallExpiresAt < ? THEN trialInstallExpiresAt
                ELSE ?
              END,
              updatedAt = ?
          WHERE shop = ?`,
